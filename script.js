...
function displayStatsTab() {
  ...
  Object.keys(averageData).forEach(className => {
    ...
    Object.keys(classData.subjects).forEach(subject => {
      ...
      tdAverageGrade.textContent = averageDataSubject.count > 0 ? (averageDataSubject.total / averageDataSubject.count).toFixed(2) : 'null';

      const tdMedianGrade = document.createElement('td');
      tdMedianGrade.textContent = medianDataSubject ? calculateMedian(medianDataSubject.grades).toFixed(2) : 'null';
